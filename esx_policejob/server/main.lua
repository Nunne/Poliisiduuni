	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				local data2 = 'Kaappi ottaminen'
				local data = xPlayer.name .. ' - '.. xPlayer.identifier ..'\nTavara: '.. inventoryItem.label..'\nMäärä: '.. count ..'x'
				TriggerEvent('esx:poliisilog', data2, data)
...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			local data2 = 'Kaappi laittaminen'
			local data = xPlayer.name .. ' - '.. xPlayer.identifier ..'\nTavara: '.. inventoryItem.label..'\nMäärä: '.. count ..'x'
			TriggerEvent('esx:poliisilog', data2, data)
...
ESX.RegisterServerCallback('esx_policejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	if removeWeapon then
		local data2 = 'Aseen tallettaminen'
		local data = xPlayer.name ..' - '.. xPlayer.identifier ..' Talletti aseen\nAse: '.. weaponName
		TriggerEvent('esx:poliisilog', data2, data)
...
ESX.RegisterServerCallback('esx_policejob:removeArmoryWeapon', function(source, cb, weaponName)
	...
	local data2 = 'Aseen ottaminen'
	local data = xPlayer.name ..' - '.. xPlayer.identifier ..' Otti aseen\nAse: '.. weaponName
	TriggerEvent('esx:poliisilog', data2, data)
	...
	if not selectedWeapon then
		...
		if type == 1 then
			if xPlayer.getMoney() >= selectedWeapon.price then
				...
				local data2 = 'Aseiden osto'
				local data =  xPlayer.name ..' - '.. xPlayer.identifier ..' Osti aseen\nAse: '.. weaponName
				TriggerEvent('esx:poliisilog', data2, data)
		...
		elseif type == 2 then
			...
			if component then
				if xPlayer.getMoney() >= price then
					...
					local data2 = 'Aseiden osien osto'
					local data =  xPlayer.name ..' - '.. xPlayer.identifier ..' Osti aseen osan\nAseeseen: '.. weaponName ..'\nAseen osa: '.. component.name
					TriggerEvent('esx:poliisilog', data2, data)
